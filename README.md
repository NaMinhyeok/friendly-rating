...
이력은 수정할 수 없는 기록으로 보존합니다. 각 점수 변경에는 두 참가자가 시간순
댓글을 남길 수 있습니다. 공유 일기는 점수와 독립된 기록이며 작성자만 자신의 글을
수정하거나 삭제할 수 있습니다. 각 일기에는 이미지 최대 네 장 또는 영상 한 개를
첨부할 수 있습니다.
...
이미지 최대 네 장 또는 짧은 영상 한 개를 첨부할 수 있습니다. 공유 일기도 댓글과
같은 첨부 정책을 사용합니다. 실제 파일은 Django나 Railway를 통과하지 않고 브라우저가
짧은 유효기간의 서명된 `PUT` URL로 Cloudflare R2에 직접 전송합니다. PostgreSQL에는
참가자 소유권, 글이나 대화 연결, 파일 종류와 크기 같은 메타데이터만 저장합니다.

파일을 선택하면 미리보기를 먼저 표시하고 이미지와 영상을 같은 흐름으로 백그라운드에
업로드합니다. 점수, 댓글 또는 공유 일기를 최종 제출할 때 업로드가 진행 중이면 같은
작업의 완료를 기다리고, 완료된 미디어 ID만 요청에 연결합니다. 업로드가 실패해도
선택은 유지하며 최종 제출에서 다시 시도합니다. 제출 전에 파일을 제거하거나 교체하면
진행 중인 전송을 중단하고 서버에도 미첨부 업로드 폐기를 요청합니다.
...
도메인 테이블명은 `participant`, `diary_entry`, `relationship_score`,
`score_change`, `score_change_comment`, `push_device`, `media_attachment`입니다.
...
- `GET /api/v1/diary-entries/{id}/`: 공유 일기 한 편 조회
