...
홈은 인증된 Django HTML 셸을 렌더링한 뒤 관계 점수를 조회 API로 채우고, 점수
기록도 JSON API로 전송합니다. `/score/change/` HTML mutation은 제거되었으며
JavaScript를 사용할 수 없으면 홈에서 점수를 조회하거나 변경할 수 없습니다. 조회
응답은 `success.results`에 현재 참가자의 outgoing score를 먼저 반환하고 각 항목의
`isMine`으로 변경 가능한 방향을 명시합니다. 인증된 관계 데이터에는
