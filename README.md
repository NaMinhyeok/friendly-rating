...
업로드합니다. 점수나 댓글을 최종 제출할 때 업로드가 진행 중이면 같은 작업의 완료를
기다리고, 완료된 미디어 ID만 요청에 연결합니다. 업로드가 실패해도 선택은 유지하며
최종 제출에서 다시 시도합니다. 제출 전에 파일을 제거하거나 교체하면 진행 중인 전송을
중단하고 서버에도 미첨부 업로드 폐기를 요청합니다.
...
완료됐지만 점수나 댓글에 연결되지 않은 파일은 24시간 뒤 만료됩니다.
...
본문만 받으며 게시 시각과 작성자는 각각 서버 시각과 로그인 세션에서 결정합니다.
