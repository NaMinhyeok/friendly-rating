...
두 참가자가 서로의 친밀도 점수와 공유 일기를 기록하는 소규모 Django
애플리케이션입니다. 참가자 이름과 4자리 PIN은 환경 변수로 관리하며, 점수 변경
이력은 수정할 수 없는 기록으로 보존합니다. 각 점수 변경과 공유 일기에는 두 참가자가
시간순 댓글을 남길 수 있습니다. 공유 일기는 점수와 독립된 기록이며 작성자만 자신의
글을 수정하거나 삭제할 수 있습니다. 각 일기에는 이미지 최대 네 장 또는 영상 한 개를
첨부할 수 있습니다.
...
이미지 최대 네 장 또는 짧은 영상 한 개를 첨부할 수 있습니다. 공유 일기 본문도 점수
대화 댓글과 같은 첨부 정책을 사용하며, 일기 댓글은 텍스트만 받습니다. 실제 파일은
Django나 Railway를 통과하지 않고 브라우저가 짧은 유효기간의 서명된 `PUT` URL로
Cloudflare R2에 직접 전송합니다. PostgreSQL에는 참가자 소유권, 글이나 대화 연결,
파일 종류와 크기 같은 메타데이터만 저장합니다.

파일을 선택하면 미리보기를 먼저 표시하고 이미지와 영상을 같은 흐름으로 백그라운드에
업로드합니다. 점수, 점수 대화 댓글 또는 공유 일기 본문을 최종 제출할 때 업로드가
진행 중이면 같은 작업의 완료를 기다리고, 완료된 미디어 ID만 요청에 연결합니다.
업로드가 실패해도 선택은 유지하며 최종 제출에서 다시 시도합니다. 제출 전에 파일을
제거하거나 교체하면 진행 중인 전송을 중단하고 서버에도 미첨부 업로드 폐기를
요청합니다.
...
수 있습니다. 브라우저의 폐기 요청이 네트워크 문제 등으로 전달되지 않은 경우를 위해,
완료됐지만 점수, 댓글 또는 공유 일기에 연결되지 않은 파일은 24시간 뒤 만료됩니다.
...
도메인 테이블명은 `participant`, `diary_entry`, `diary_entry_comment`,
`relationship_score`, `score_change`, `score_change_comment`, `push_device`,
`media_attachment`입니다.
...
- `GET /api/v1/diary-entries/`: 두 참가자가 공유하는 일기 목록 조회
- `POST /api/v1/diary-entries/`: 로그인한 참가자의 공유 일기 작성
- `GET /api/v1/diary-entries/{id}/`: 공유 일기 한 편과 시간순 댓글 조회
- `PATCH /api/v1/diary-entries/{id}/`: 작성자가 자신의 공유 일기 수정
- `DELETE /api/v1/diary-entries/{id}/`: 작성자가 자신의 공유 일기 삭제
- `POST /api/v1/diary-entries/{id}/comments/`: 로그인한 참가자의 일기 댓글 작성
- `POST /api/v1/media-uploads/`: 비공개 R2 직접 업로드 URL 발급
- `POST /api/v1/media-uploads/{id}/complete/`: 업로드된 파일 확인 및 첨부 준비
- `POST /api/v1/media-uploads/{id}/discard/`: 미첨부 업로드 전송 취소 및 즉시 폐기
...
홈, 공유 일기, 마음 기록과 점수 대화는 인증된 Django HTML 셸을 렌더링한 뒤 JSON
API로 데이터를 채웁니다. 홈의 점수 변경도 API로 전송하며 `/score/change/` HTML
mutation은 제거되었습니다. JavaScript를 사용할 수 없으면 이 화면들에서 관계
데이터를 조회하거나 변경할 수 없습니다. 공유 일기는 서버가 기록한 실제 게시 시각의
최신순으로 조회하며 여러 편을 연달아 남길 수 있습니다. 작성 요청은 1,000자 이하의
본문을 필수로 받고 선택한 첨부 업로드 ID도 함께 받을 수 있습니다. 게시 시각과
작성자는 각각 서버 시각과 로그인 세션에서 결정합니다. 일기를 수정할 때 첨부 ID를
생략하면 기존 파일을 유지하고, 전달하면 해당 순서의 최종 첨부 목록으로 교체합니다.
일기 목록은 각 글의 댓글 수와 일기 대화 화면 경로를 포함하며, 일기 대화 화면은
본문과 평평한 시간순 댓글을 함께 조회합니다. 두 참가자 모두 500자 이하의 텍스트
댓글을 남길 수 있고 작성자는 로그인 세션에서 결정됩니다. 글을 삭제하면 그 글의
댓글도 함께 삭제됩니다.
관계 점수 응답은 `success.results`에 현재 참가자의 outgoing score를 먼저 반환하고
각 항목의 `isMine`으로 변경 가능한 방향을 명시합니다. 점수 변경 이력은
`pageNumber`로 조회하고 20개 고정 크기의
`success.results`와 `success.paging`을 반환합니다. 각 이력은 댓글 수와 점수 대화
화면 경로를 포함합니다. 점수 대화와 일기 대화 화면은 최초 진입,
백그라운드·BFCache 복귀와 활성 화면의 관련 푸시 수신 때 API에서 최신 댓글을 다시
조회합니다. 점수 변경·점수 댓글·일기 댓글 푸시는 사적인 내용 없이 해당 대화 화면으로
연결됩니다. 인증된 관계 데이터에는 `Cache-Control: private, no-store`가 적용됩니다.
